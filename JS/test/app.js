document.addEventListener("DOMContentLoaded", () => {
  ...
  let currentDate = new Date("2025-05-19"); // Updated to current date
  let availability = {
  ...
  function getUniqueTimeSlot(dayName) {
    ...
    if (existingSlots.length > 0) {
      ...
      if (lastEndTime < 18) {
        ...
        const newSlot = { start, end };
        if (isValidSlot(newSlot) && !existingSlots.some(slot => isOverlapping(slot, newSlot))) {
          return newSlot;
        }
      }
    }
  ...
  function updateAvailabilityStatus(dayRow, dayName) {
    ...
    status.textContent = availability[dayName] && availability[dayName].length ? "متاح" : "غير متاح";
  }
  ...
  saveBtn.addEventListener("click", () => {
    ...
    // Extend  Extend with localStorage or API call here
  ...
  document.querySelectorAll(".availability-row").forEach(row => {
    ...
    row.querySelector(".add-btn").addEventListener("click", () => {
      ...
      // Rebuild time slots to reflect sorted order
      ...
      timeSlotsDiv.querySelectorAll(".time-slot").forEach((slotElement, index) => {
        ...
        startInput.addEventListener("change", () => {
          ...
          if (!isValidSlot(updatedSlot)) {
            ...
            startInput.value = availability[dayName][index].start; // Revert
            return;
          }

          // Check for overlaps
          const hasOverlap = availability[dayName].some((slot, i) => 
            i !== index && isOverlapping(slot, updatedSlot)
          );
          if (hasOverlap) {
          ...
          availability[dayName].forEach(slot => {
            ...
            newSlotElement.className = "time-slot";
            newSlotElement.innerHTML = `
              ...
              <button class="remove-slot-btn">×</button>
            `;
            timeSlotsDiv.appendChild(newSlotElement);
          });
          renderCalendar();
        });
        ...
        endInput.addEventListener("change", () => {
          ...
          if (!isValidSlot(updatedSlot)) {
            alert("وقت الانتهاء يجب أن يكون بعد وقت البدء");
            endInput.value = availability[dayName][index].end; // Revert
            return;
          }
